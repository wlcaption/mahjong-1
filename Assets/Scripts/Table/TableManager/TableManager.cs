...
public class TableManager<T, U> : Singleton<U>, ITableManager where T : ITableItem where U : new() {

    // the data arrays.
    private T[] mItemArray;
    private Dictionary<int, int> mKeyItemMap = new Dictionary<int, int>();

    public virtual string TableName() { return string.Empty; }
    ...
    public void OnWriteField(T[] configs, string TableName) {
        ...
        FileInfo fi = new FileInfo(UnityEngine.Application.dataPath + "/Table/" + "//" + TableName + ".txt");
        if (!fi.Exists) {
            ...
            WWW www = new WWW(UnityEngine.Application.dataPath + "/Table/" + TableName + ".txt");
        ...
        CreateOrOPenFile(UnityEngine.Application.dataPath + "/Table/", TableName + ".txt", builders.ToString());
    }
